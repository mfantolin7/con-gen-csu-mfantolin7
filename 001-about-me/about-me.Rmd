...
My favorite paper in all of evolutionary biology is @gould1979philosophical "Spandrels of San Marco and the Panglossian Paradigm."  This rhetorical masterpiece examines how to study adaptation, with the idea that many traits in organisms may not be adaptations per se, but may result as byproducts phylogenetic constraints contingent on the body plans within particular lineages.     
...
|     $H_{T}$ is the total heterozygosity across all populations, 
...
|
|
A second equation that we use measures the mean kinship $\bar{k}_i$ of each individual (based on relatedness between individuals based on pedigrees) to the $n-1$ other individuals in a captive-breeding population.  
...
I learned to program in FORTRAN back in 1983.  This was useful for the logical necessities of coding in other programs.  I have used R since abut 2004, and in 2005 took a spatial statistics course that used R (S-plus actually, but let's not quibble).  
...
This is R-code written to analyze some immunological assays for testing a plague vaccine in Black-footed ferrets.  This was part of Debbie McCuen's MS thesis.   
...
## Calculate student's t distribution value, t(alpha/2)=2.31 for df 8, t(alpha/2)=2.11 for df 17, t(alpha/2)=2.78 for df 4 (to calculate CIs)
qt((1-0.05/2), 8)
qt((1-0.05/2), 17)
qt((1-0.05/2), 4)

##Import data, R will prompt you for the filename
vacvstrt <- read.csv(file.choose (), quote = "'") 

# HI_Treatment_vs_vaccine_type.csv
str(vacvstrt)

## Make Individual a factor
vacvstrt$Individual<-as.factor(vacvstrt$Individual)
is.factor(vacvstrt$Individual)

##Set contrasts option in R that sums to zero 
options(contrasts = c("contr.sum", "contr.poly"))

View(vacvstrt)

# run a two-way ANOVA with interactions - Type 1 ANOVA table 
Two_way_interaction<-lm(Viral_load ~ Vaccine_type * HI_Treatment, data = vacvstrt)
anova(Two_way_interaction)
Two_way_interaction

#re-run without interaction
Two_way<-lm(Viral_load ~ Vaccine_type + HI_Treatment, data = vacvstrt)
anova(Two_way)
Two_way

# USE THIS
# Re-run with interaction, but with indivudual, the "paired t-test" version of the 2-way ANOVA)
Two_way_individual<-lm(Viral_load ~ Vaccine_type * HI_Treatment + Individual, data = vacvstrt)
anova(Two_way_individual)
Two_way_individual

# Re-run without interaction, but with individual, the "paired t-test" version of the 2-way ANOVA)
Two_way_individual<-lm(Viral_load ~ Vaccine_type + HI_Treatment + Individual, data = vacvstrt)
anova(Two_way_individual)
Two_way_individual
...
#plot the data
with(interaction.plot(Vaccine_type, HI_Treatment, Viral_load),data=vacvstrt)

## Run Type 3 ANOVA table 
Model2 <- lm(Viral_load ~ Vaccine_type*HI_Treatment, data = vacvstrt)
Anova(Model2, type = 3)

lsmeans(Model2, pairwise ~ Vaccine_type)

lsmeans(Model2, pairwise ~ HI_Treatment)

lsmeans(Model2, pairwise ~ Vaccine_type|HI_Treatment)

## boxplot F1V vs vaccinia
boxplot(Viral_load ~ Vaccine_type, data = vacvstrt)

## boxplot HI vs non-HI
boxplot(Viral_load ~ HI_Treatment, data = vacvstrt)

library(lattice)
library(gplots)
library(ggplot2)

## nicer boxplot HI vs non-HI


p <-qplot(HI_Treatment, Viral_load, geom = "boxplot", data = vacvstrt) + xlab("HI Treatment") + ylab("Viral Plaque Percentage")   
p

##nicer boxplot F1V vs vaccinia
q <-qplot(Vaccine_type, Viral_load, geom = "boxplot", data = vacvstrt) + xlab("Vaccine Type") + ylab("Viral Plaque Percentage") 
q

## wild ferret data
wildNONHI = c(88.89, 56.35, 41.3, 56.35, 46.03)
wildHI = c(49.2, 38.1, 63.1, 46.03, 71.03)

## wild ferret paired t-test HI vs Non-HI, difference of means between each group? NO
t.test(wildHI, wildNONHI, paired = T)
```
